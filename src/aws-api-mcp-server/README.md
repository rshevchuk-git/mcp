...
| `AWS_API_MCP_TELEMETRY` | ❌ No | `"true"` | Controls whether the server sends additional telemetry data to AWS about its configuration and usage. When enabled, this includes:<br></br> - Whether the `call_aws()` tool is used with `READ_OPERATIONS_ONLY` set to true or false.<br></br>Note: Regardless of this setting, AWS obtains information about which operations were invoked and the server version as part of normal AWS service interactions; no additional telemetry calls are made by the server for this purpose. |
