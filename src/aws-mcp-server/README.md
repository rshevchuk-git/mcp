...
## Prerequisites
- Have an AWS account with [credentials configured](https://boto3.amazonaws.com/v1/documentation/api/latest/guide/credentials.html#configuring-credentials)
...
If you prefer to use `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, and `AWS_SESSION_TOKEN` as environment variables, add them to the "env" section. Make sure to remove `AWS_PROFILE` to avoid conflicts.
...
#### Optional
- `AWS_PROFILE` (string, default: "default"): AWS Profile for credentials to use for command executions, 'default' will be used if not specified. Make sure to NOT set this if you're using environment variables'
- `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` and `AWS_SESSION_TOKEN`: Use environement variables to configure credentials, mutually exclusive with `AWS_PROFILE`
